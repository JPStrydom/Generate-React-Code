...
function getAllDirectories(name, directory, native, redux, reduxCore, reduxCoreDirectory, functional) {
    if (reduxCore) {
        return getReduxCoreDirs(reduxCoreDirectory);
    }
    return getReactComponentDirs(name, directory, native, redux, functional);
}
...
function getReactComponentDirs(name, directory, native, redux, functional) {
    ...
    const reactDirs = (functional === true) ?
        {
            functional: {
                template: path.join(templateDirectory, 'template.view.functional.js'),
                generated: path.join(generatedDirectory, `${name}.view.funcational.js`)
            }
        } : {
            view: {
    ...
    const reduxDirs = {
        ...
        action: {
            template: path.join(templateDirectory, 'template.action.js'),
            generated: path.join(generatedDirectory, `${name}.action.js`)
        },
        reducer: {
